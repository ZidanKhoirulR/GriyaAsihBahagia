...
interface Warga {
    ...
    nama_bayi: string;
    usia_bayi_bulan: number;
    tanggal_lahir_bayi: string;
    kartu_keluarga?: { rt: string; rw: string; nomor_kk: string };
}

export default function IbuMenyusui({ wargas, totalData, tanggalAcuan, eksporUrl }: { wargas: Warga[]; totalData: number; tanggalAcuan: string; eksporUrl: string }) {
    const [searchTerm, setSearchTerm] = useState('');

    const filtered = wargas.filter(w => {
        if (!searchTerm) return true;
        const s = searchTerm.toLowerCase();
        return w.nama_lengkap.toLowerCase().includes(s) || w.nik.includes(s) || w.nama_bayi?.toLowerCase().includes(s);
    });
    ...
    return (
        <PengurusRTLayout title="Kelompok Sasaran – Ibu Menyusui">
            <Head title="Data Ibu Menyusui" />

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                {/* Card Total Ibu Menyusui – gaya dashboard */}
                <div className="bg-white rounded-xl p-3 md:p-4 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] flex items-center justify-between hover:-translate-y-1 transition-transform duration-300">
                    <div className="flex items-center gap-2.5">
                        <div className="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0 shadow-md shadow-blue-500/30">
                            <img src="/images/icons/logoibumenyusui.png" alt="Ibu Menyusui" className="w-7 h-7 object-contain" />
                        </div>
                        <div className="text-left">
                            <p className="font-bold text-gray-900 leading-tight">Total Ibu Menyusui</p>
                        </div>
                    </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 className="text-sm font-bold text-gray-800 mb-2">Panduan Kelompok Ibu Menyusui</h3>
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    <li>Menampilkan warga perempuan yang memiliki bayi berusia <strong>0 – 24 bulan</strong> dalam satu KK yang sama.</li>
                    <li>Deteksi dilakukan otomatis berdasarkan tanggal lahir bayi dalam satu Kartu Keluarga (KK).</li>
                    <li>Ibu <strong>otomatis keluar</strong> dari daftar ini saat bayi berusia lebih dari 24 bulan.</li>
                    <li>Pastikan data lahir bayi sudah terdaftar di KK yang sama dengan ibunya.</li>
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-between mb-4">
                    <div className="relative w-full sm:w-80">
                        ...
                        <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                            className="block w-full p-2 pl-9 text-sm border border-gray-300 rounded-lg bg-gray-50 focus:ring-pink-500 focus:border-pink-500"
                            placeholder="Cari nama ibu, NIK, atau nama bayi..." />
                    </div>
                    ...
                    <table className="w-full text-xs text-left text-gray-500">
                        <thead className="text-[10px] text-gray-700 uppercase bg-gray-50/80 border-y border-gray-200">
                            <tr>
                                ...
                                <th className="px-3 py-3 text-center">Nama Bayi</th>
                                <th className="px-3 py-3 text-center">Usia Bayi</th>
                                <th className="px-3 py-3 text-center">No. WhatsApp</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr><td colSpan={8} className="px-3 py-8 text-center text-gray-500">Tidak ada data ibu menyusui ditemukan.</td></tr>
                            ) : (
                                currentData.map((w, i) => (
                                    <tr key={w.id} className="bg-white border-b hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-3 py-2 text-center capitalize">{w.nama_bayi || '-'}</td>
                                        <td className="px-3 py-2 text-center">{w.usia_bayi_bulan} bln</td>
