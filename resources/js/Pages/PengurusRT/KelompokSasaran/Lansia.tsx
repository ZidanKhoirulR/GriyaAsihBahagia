...
export default function Lansia({ wargas, totalData, tanggalAcuan, eksporUrl }: { wargas: Warga[]; totalData: number; tanggalAcuan: string; eksporUrl: string }) {
    ...
    const getUsiaBadgeColor = (usia: number) => {
        if (usia >= 80) return 'bg-red-50 text-red-600';
        if (usia >= 70) return 'bg-orange-50 text-orange-600';
        return 'bg-amber-50 text-amber-600';
    };

    return (
        <PengurusRTLayout title="Kelompok Sasaran – Lansia">
            <Head title="Data Lansia" />

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                {/* Card Total Lansia – gaya dashboard */}
                <div className="bg-white rounded-xl p-3 md:p-4 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] flex items-center justify-between hover:-translate-y-1 transition-transform duration-300">
                    <div className="flex items-center gap-2.5">
                        <div className="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0 shadow-md shadow-blue-500/30">
                            <img src="/images/icons/logolansia.png" alt="Lansia" className="w-7 h-7 object-contain" />
                        </div>
                        <div className="text-left">
                            <p className="font-bold text-gray-900 leading-tight">Total Lansia</p>
                        </div>
                    </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 className="text-sm font-bold text-gray-800 mb-2">Panduan Kelompok Lansia</h3>
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    <li>Menampilkan seluruh warga yang berusia <strong>≥ 60 tahun</strong> per tanggal acuan.</li>
                    <li>Badge usia berwarna <span className="font-semibold text-amber-600">kuning</span> (60-69 thn), <span className="font-semibold text-orange-600">oranye</span> (70-79 thn), dan <span className="font-semibold text-red-600">merah</span> (≥80 thn).</li>
                    <li>Warga yang sudah meninggal tidak ditampilkan dalam daftar ini.</li>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div className="overflow-x-auto rounded-xl border border-gray-100">
                    <table className="w-full text-xs text-left text-gray-500">
                        ...
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr><td colSpan={9} className="px-3 py-8 text-center text-gray-500">Tidak ada data lansia ditemukan.</td></tr>
                            ) : (
                                currentData.map((w, i) => (
                                    <tr key={w.id} className="bg-white border-b hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-3 py-2 text-center">
                                            <span className={`px-2 py-0.5 rounded text-xs font-medium ${getUsiaBadgeColor(w.usia)}`}>{w.usia} thn</span>
                                        </td>
                                        <td className="px-3 py-2 text-center">{w.agama || '-'}</td>
