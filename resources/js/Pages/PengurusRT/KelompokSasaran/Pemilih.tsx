...
export default function Pemilih({ wargas, totalData, tanggalAcuan, eksporUrl }: { wargas: Warga[]; totalData: number; tanggalAcuan: string; eksporUrl: string }) {
    ...
    return (
        <PengurusRTLayout title="Kelompok Sasaran – Pemilih">
            <Head title="Data Pemilih" />

            {/* Header Stats */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                {/* Card Total Pemilih – gaya dashboard */}
                <div className="bg-white rounded-xl p-3 md:p-4 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] flex items-center justify-between hover:-translate-y-1 transition-transform duration-300">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div className="text-left">
                            <p className="font-bold text-gray-900 leading-tight">Total Pemilih</p>
                        </div>
                    </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 className="text-sm font-bold text-gray-800 mb-2">Panduan Kelompok Pemilih</h3>
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    <li>Menampilkan seluruh warga yang berusia <strong>≥ 17 tahun</strong> per tanggal acuan.</li>
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-between mb-4">
                    <div className="relative w-full sm:w-80">
                        ...
                        <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                            className="block w-full p-2 pl-9 text-sm border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <table className="w-full text-xs text-left text-gray-500">
                        <thead className="text-[10px] text-gray-700 uppercase bg-gray-50/80 border-y border-gray-200">
                            <tr>
                                ...
                                <th className="px-3 py-3 text-center">Status Perkawinan</th>
                                <th className="px-3 py-3 text-center">No. WhatsApp</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr><td colSpan={8} className="px-3 py-8 text-center text-gray-500">Tidak ada data pemilih ditemukan.</td></tr>
                            ) : (
                                currentData.map((w, i) => (
                                    <tr key={w.id} className="bg-white border-b hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-3 py-2 text-center">{w.status_perkawinan || '-'}</td>
