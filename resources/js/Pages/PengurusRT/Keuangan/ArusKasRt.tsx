...
export default function ArusKasRt({ transaksi, metode, totalPemasukan, totalPengeluaran, saldo, currentRt }: {
    transaksi: Transaksi[]; metode: Metode[];
    totalPemasukan: number; totalPengeluaran: number; saldo: number; currentRt: string;
}) {
    ...
    const { data, setData, post, reset, errors, clearErrors } = useForm({
        tanggal: '', rt: currentRt || '', kategori: '', metode_transaksi_id: '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const routeName = editingTrx ? route('pengurus-rt.arus-kas-rt.update', editingTrx.id) : route('pengurus-rt.arus-kas-rt.store');
        post(routeName, {
    ...
    const confirmDelete = () => {
        ...
        router.delete(route('pengurus-rt.arus-kas-rt.destroy', selectedTrx.id), {
    ...
    const confirmValidasi = () => {
        if (!selectedTrx) return;
        router.patch(route('pengurus-rt.arus-kas-rt.validasi', selectedTrx.id), {}, {
    ...
    return (
        <PengurusRTLayout title="Keuangan - Arus Kas RT">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {transaksi.map((t, i) => (
                                    <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-2 py-3">{t.metode_transaksi?.nama_metode || (t.iuran_warga_id ? 'Transfer' : '-')}</td>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" className="w-full rounded-lg border-gray-300 shadow-sm bg-gray-100 text-gray-500 cursor-not-allowed" value={data.rt} disabled required />
