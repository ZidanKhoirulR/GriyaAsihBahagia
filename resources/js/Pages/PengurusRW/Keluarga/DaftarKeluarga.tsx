...
export default function DaftarKeluarga({ keluargas, rumahs, eksporUrl }: { keluargas: KeluargaItem[], rumahs: RumahItem[], eksporUrl: string }) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        post(route('pengurus-rw.keluarga.store'), {
    ...
    const submitEdit: FormEventHandler = (e) => {
        ...
        editForm.post(route('pengurus-rw.keluarga.update', selectedKeluarga.encrypted_id), {
    ...
    const confirmDelete = () => {
        if (!selectedKeluarga) return;
        router.delete(route('pengurus-rw.keluarga.destroy', selectedKeluarga.encrypted_id), {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="grid grid-cols-4 gap-4">
                        <div className="col-span-1">
                            <InputLabel htmlFor="rt" value="Pilih RT" className="!text-xs" required />
                            <select id="rt" className={selectClass} value={formData.rt} onChange={e => {
                                setFormData('rt', e.target.value);
                                setFormData('rumah_id', ''); // reset rumah_id if RT changes
                            }} required>
                                <option value="">Pilih RT</option>
                                {uniqueRTs.map(rtValue => (
                                    <option key={rtValue} value={rtValue}>{rtValue}</option>
                                ))}
                            </select>
    ...
    return (
        <PengurusRWLayout title="Data Keluarga">
                                        ...
                                        <tr key={keluarga.id} className="bg-white border-b hover:bg-gray-50 transition-colors text-center">
                                            ...
                                            <td className="px-3 py-2 text-center">
                                                <div className="flex items-center justify-center gap-1.5 whitespace-nowrap">
                                                    ...
                                                    <Link href={route('pengurus-rw.warga.index', { keluarga: keluarga.encrypted_id })} className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded hover:bg-emerald-200 transition-colors font-medium">
                                                        Kelola
                                                    </Link>
                                                </div>
                                            </td>
