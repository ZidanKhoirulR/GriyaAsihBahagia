...
const KATEGORI_PEMASUKAN = ['Iuran Warga', 'Iuran RT', 'Dana Bantuan', 'Sumbangan', 'Lain-lain'];
const KATEGORI_PENGELUARAN = ['Operasional RW', 'Kegiatan Warga', 'Perbaikan Fasilitas', 'Dana Sosial', 'Administrasi', 'Lain-lain'];

const fmt = (n: number) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(n);

export default function ArusKasRw({ transaksi, metode, totalPemasukan, totalPengeluaran, saldo }: {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const routeName = editingTrx ? route('pengurus-rw.arus-kas-rw.update', editingTrx.id) : route('pengurus-rw.arus-kas-rw.store');
        post(routeName, {
    ...
    const confirmDelete = () => {
        ...
        router.delete(route('pengurus-rw.arus-kas-rw.destroy', selectedTrx.id), {
    ...
    const confirmValidasi = () => {
        if (!selectedTrx) return;
        router.patch(route('pengurus-rw.arus-kas-rw.validasi', selectedTrx.id), {}, {
    ...
    return (
        <PengurusRWLayout title="Keuangan - Arus Kas RW">
            <Head title="Arus Kas RW" />

            {/* Panduan */}
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 className="text-sm font-bold text-gray-800 mb-2">Panduan Pengelolaan Arus Kas RW</h3>
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    <li><span className="inline-block px-2 py-0.5 bg-amber-500 rounded text-xs font-medium text-white">+ Tambah Transaksi</span>: Catat pemasukan atau pengeluaran kas RW beserta slip/struk pembayaran.</li>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-xs text-center text-gray-500">
                            <thead className="text-[10px] text-gray-700 font-semibold bg-gray-50">
                                <tr>
                                    {['No','Tanggal','RT','Kategori','Metode','Pemasukan','Pengeluaran','Slip','Penyetor','Penerima','Catatan','Status','Aksi'].map((h, i) => (
                                        <th key={h} className={`px-2 py-2 ${i === 0 ? 'rounded-tl-lg' : i === 12 ? 'rounded-tr-lg' : ''}`}>{h}</th>
                                    ))}
                                </tr>
                            </thead>
            ...
            <Modal show={isModalOpen} onClose={() => setIsModalOpen(false)}>
                <div className="p-6">
                    <h3 className="text-lg font-bold text-gray-900 mb-4">{editingTrx ? 'Edit Transaksi' : 'Tambah Transaksi Kas RW'}</h3>
