...
export default function MetodeTransaksi({ metode }: { metode: MetodeTransaksi[] }) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingMetode) {
            put(route('pengurus-rw.metode-transaksi.update', editingMetode.id), {
            ...
            post(route('pengurus-rw.metode-transaksi.store'), {
    ...
    const confirmDelete = () => {
        if (selectedForDelete) {
            router.delete(route('pengurus-rw.metode-transaksi.destroy', selectedForDelete.id), {
    ...
    return (
        <PengurusRWLayout title="Keuangan - Metode Transaksi">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                ...
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    ...
                    <li className="leading-relaxed">
                        Atur status <span className="inline-block px-2 py-0.5 bg-green-100 rounded text-xs font-medium text-green-700 cursor-default align-baseline">Aktif</span> agar metode tersedia untuk dipilih saat mencatat transaksi.
                    </li>
                </ul>
            </div>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {metode.map((m, i) => (
                                    <tr key={m.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-3 py-3 text-left max-w-xs truncate">{m.keterangan || '-'}</td>
                                        <td className="px-3 py-3">
                                            <span className={`px-2.5 py-1 rounded-full text-[10px] font-medium ${m.is_aktif ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
