...
function hitungUsiaWafat(tglLahir: string | null, tglWafat: string | null) {
    if (!tglLahir || !tglWafat) return '-';
    const lahir = new Date(tglLahir);
    const wafat = new Date(tglWafat);
    let usia = wafat.getFullYear() - lahir.getFullYear();
    const m = wafat.getMonth() - lahir.getMonth();
    if (m < 0 || (m === 0 && wafat.getDate() < lahir.getDate())) usia--;
    return `${usia} Tahun`;
}

export default function Kematian({ auth, warga, eksporUrl }: PageProps<{ warga: Warga[]; eksporUrl: string }>) {
    ...
    return (
        <PengurusRWLayout title="Data Kematian Warga">
            <Head title="Data Kematian Warga" />

            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 className="text-sm font-bold text-gray-800 mb-2">Mutasi: Kematian</h3>
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    <li className="leading-relaxed">
                        Halaman ini hanya untuk melihat rekapan data kematian warga.
                    </li>
                    <li className="leading-relaxed">
                    ...
                    <table className="w-full text-xs text-center text-gray-500">
                        <thead className="text-[10px] text-gray-700 uppercase font-semibold bg-gray-50 border-y border-gray-200">
                            <tr>
                                ...
                                <th scope="col" className="px-3 py-2 text-center">Tempat, Tanggal Wafat</th>
                                <th scope="col" className="px-3 py-2 text-center">Usia Meninggal</th>
                            ...
                            {filteredWarga.length > 0 ? (
                                currentData.map((item, index) => (
                                    <tr key={item.id} className="bg-white border-b hover:bg-gray-50 transition-colors text-center">
                                        ...
                                        <td className="px-3 py-2 whitespace-nowrap">
                                            {item.tempat_meninggal ? `${item.tempat_meninggal}, ${formatTanggal(item.tanggal_meninggal)}` : '-'}
                                        </td>
                                        <td className="px-3 py-2 whitespace-nowrap">
                                            {hitungUsiaWafat(item.tanggal_lahir, item.tanggal_meninggal)}
                                        </td>
                                ...
                                <tr>
                                    <td colSpan={9} className="px-3 py-6 text-center text-gray-500">
                                        {searchTerm ? 'Data kematian tidak ditemukan.' : 'Belum ada data kematian.'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
