...
export default function PindahPergi({ auth, warga, eksporUrl }: PageProps<{ warga: Warga[]; eksporUrl: string }>) {
    ...
    return (
        <PengurusRWLayout title="Data Warga Pindah Pergi">
            <Head title="Data Warga Pindah Pergi" />

            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 className="text-sm font-bold text-gray-800 mb-2">Mutasi: Pindah Pergi</h3>
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    <li className="leading-relaxed">
                        Halaman ini hanya untuk melihat rekapan data pindah pergi warga.
                    </li>
                    <li className="leading-relaxed">
                    ...
                    <table className="w-full text-xs text-center text-gray-500">
                        <thead className="text-[10px] text-gray-700 uppercase font-semibold bg-gray-50 border-y border-gray-200">
                            <tr>
                                ...
                                <th scope="col" className="px-3 py-2 text-center">Tanggal Pindah Pergi</th>
                                <th scope="col" className="px-3 py-2 text-center">Pindah Pergi Ke</th>
                            ...
                            {filteredWarga.length > 0 ? (
                                currentData.map((item, index) => (
                                    <tr key={item.id} className="bg-white border-b hover:bg-gray-50 transition-colors text-center">
                                        ...
                                        <td className="px-3 py-2 whitespace-nowrap">{formatTanggal(item.tanggal_pindah_pergi)}</td>
                                        <td className="px-3 py-2">{item.pindah_pergi || '-'}</td>
                                ...
                                <tr>
                                    <td colSpan={11} className="px-3 py-6 text-center text-gray-500">
                                        {searchTerm ? 'Data pindah pergi tidak ditemukan.' : 'Belum ada data warga pindah pergi.'}
                                    </td>
                                </tr>
                            )}
                        </tbody>
