...
export default function PengurusRT({ wargas, totalData, totalPengurus }: { wargas: Warga[]; totalData: number; totalPengurus: number }) {
    ...
    const [jabatanForm, setJabatanForm] = useState<JabatanForm>({ jabatan: '', rt: '', periode_mulai: '', periode_selesai: '' });
    ...
    const openModal = (w: Warga) => {
        setModalWarga(w);
        if (w.is_pengurus) {
            setJabatanForm({ jabatan: w.jabatan_pengurus || '', rt: w.rt_pengurus || '', periode_mulai: '', periode_selesai: '' });
        } else {
            setJabatanForm({ jabatan: '', rt: w.kartu_keluarga?.rt || '', periode_mulai: new Date().toISOString().split('T')[0], periode_selesai: '' });
        }
    };
    ...
    const handleToggle = () => {
        ...
        if (modalWarga.is_pengurus && modalWarga.pengurus_id) {
            router.patch(route('pengurus-rw.kelola-pengurus.toggle', modalWarga.pengurus_id), {}, {
                onSuccess: () => { showToast('Pengurus RT berhasil dinonaktifkan'); setModalWarga(null); },
            ...
            router.post(route('pengurus-rw.kelola-pengurus.store'), {
                ...
                onSuccess: () => { showToast('Warga berhasil dijadikan Pengurus RT'); setModalWarga(null); },
    ...
    return (
        <PengurusRWLayout title="Kelola Pengurus — RT">
            <Head title="Pengurus RT" />

            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 className="text-sm font-bold text-gray-800 mb-2">Panduan Kelola Pengurus RT</h3>
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    ...
                    <li>Gunakan <strong>toggle</strong> di kolom Status untuk menjadikan atau melepas jabatan warga sebagai Pengurus RT.</li>
                    <li>Saat mengaktifkan, isi <strong>jabatan</strong> dan <strong>nomor RT</strong> terlebih dahulu.</li>
                    <li>Gunakan filter <strong>Pengurus Aktif</strong> untuk melihat daftar pengurus yang sedang menjabat pada RT tertentu.</li>
                </ul>
            </div>
                    ...
                    <table className="w-full text-xs text-left text-gray-500">
                        ...
                        <tbody>
                            {currentData.length === 0 ? (
                                <tr><td colSpan={10} className="px-3 py-8 text-center text-gray-500">Tidak ada data ditemukan.</td></tr>
                            ) : (
                                currentData.map((w, i) => (
                                    <tr key={w.id} className="bg-white border-b hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-3 py-2 text-center">
                                            <button
                                                onClick={() => openModal(w)}
                                                title={w.is_pengurus ? 'Nonaktifkan sebagai Pengurus RT' : 'Jadikan Pengurus RT'}
                ...
                {modalWarga && (
                    <div className="p-6">
                        {modalWarga.is_pengurus ? (
                            <>
                                <h3 className="text-base font-bold text-center text-gray-900 mb-1">Nonaktifkan Pengurus RT?</h3>
                                ...
                                <p className="text-xs text-center text-gray-400 mb-5">
                                    Jabatan: <span className="font-medium text-amber-600">{modalWarga.jabatan_pengurus}</span>
                                    {modalWarga.rt_pengurus && <span className="text-gray-500"> · RT {modalWarga.rt_pengurus}</span>}
                                </p>
                            ...
                            <>
                                <h3 className="text-base font-bold text-center text-gray-900 mb-1">Jadikan Pengurus RT</h3>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            ...
                                            <input type="text" className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500"
                                                placeholder="Ketua RT, Sekretaris..."
                                                value={jabatanForm.jabatan} onChange={e => setJabatanForm(p => ({ ...p, jabatan: e.target.value }))} />
                                        </div>
                                        <div>
                                            <label className="block text-xs font-medium text-gray-700 mb-1">No. RT <span className="text-red-500">*</span></label>
                                            <input type="text" className="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500"
                                                placeholder="Contoh: 001"
                                                value={jabatanForm.rt} onChange={e => setJabatanForm(p => ({ ...p, rt: e.target.value }))} />
                                        </div>
                                    </div>
                                </div>
                                <div className="flex justify-center gap-3 mt-5">
                                    ...
                                    <button onClick={handleToggle}
                                        disabled={isSubmitting || !jabatanForm.jabatan || !jabatanForm.rt}
