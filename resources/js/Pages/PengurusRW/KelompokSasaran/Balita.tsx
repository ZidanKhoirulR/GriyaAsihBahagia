...
export default function Balita({ wargas, totalData, tanggalAcuan, eksporUrl }: { wargas: Warga[]; totalData: number; tanggalAcuan: string; eksporUrl: string }) {
    const [searchTerm, setSearchTerm] = useState('');

    const filtered = wargas.filter(w => {
        if (!searchTerm) return true;
        const s = searchTerm.toLowerCase();
        return w.nama_lengkap.toLowerCase().includes(s) || w.nik.includes(s) || w.nama_ayah?.toLowerCase().includes(s) || w.nama_ibu?.toLowerCase().includes(s);
    });
    ...
    const formatUsia = (tahun: number, bulan: number) => {
        if (tahun === 0) return `${bulan} bulan`;
        return `${tahun} thn ${bulan % 12} bln`;
    };

    return (
        <PengurusRWLayout title="Kelompok Sasaran – Balita">
            <Head title="Data Balita" />

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                {/* Card Total Balita – gaya dashboard */}
                <div className="bg-white rounded-xl p-3 md:p-4 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] flex items-center justify-between hover:-translate-y-1 transition-transform duration-300">
                    <div className="flex items-center gap-2.5">
                        <div className="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0 shadow-md shadow-blue-500/30">
                            <img src="/images/icons/logobalita.png" alt="Balita" className="w-10 h-10 object-contain" />
                        </div>
                        <div className="text-left">
                            <p className="font-bold text-gray-900 leading-tight">Total Balita</p>
                        </div>
                    </div>
                ...
                {/* Card Laki-Laki – gaya dashboard */}
                <div className="bg-white rounded-xl p-3 md:p-4 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] flex items-center justify-between hover:-translate-y-1 transition-transform duration-300">
                    <div className="flex items-center gap-2.5">
                        <div className="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0 shadow-md shadow-blue-500/30">
                            <img src="/images/icons/logopria.png" alt="Pria" className="w-7 h-7 object-contain" />
                        </div>
                        <div className="text-left">
                    ...
                    <p className="text-xl md:text-2xl text-gray-900 tracking-tight">
                        {wargas.filter(w => w.jenis_kelamin === 'L').length.toLocaleString('id-ID')}
                    </p>
                </div>

                {/* Card Perempuan – gaya dashboard */}
                <div className="bg-white rounded-xl p-3 md:p-4 border border-gray-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] flex items-center justify-between hover:-translate-y-1 transition-transform duration-300">
                    <div className="flex items-center gap-2.5">
                        <div className="w-12 h-12 rounded-full bg-blue-600 flex items-center justify-center flex-shrink-0 shadow-md shadow-blue-500/30">
                            <img src="/images/icons/logowanita.png" alt="Wanita" className="w-7 h-7 object-contain" />
                        </div>
                        <div className="text-left">
                    ...
                    <p className="text-xl md:text-2xl text-gray-900 tracking-tight">
                        {wargas.filter(w => w.jenis_kelamin === 'P').length.toLocaleString('id-ID')}
                    </p>
                </div>
            </div>

            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 className="text-sm font-bold text-gray-800 mb-2">Panduan Kelompok Balita</h3>
                <ul className="text-sm text-gray-600 list-disc list-outside space-y-1.5 ml-5">
                    <li>Menampilkan seluruh warga yang berusia <strong>0 – 5 tahun (belum genap 6 tahun)</strong> per tanggal acuan.</li>
                    <li>Kolom <strong>&quot;Usia&quot;</strong> menampilkan usia dalam bulan jika balita belum genap 1 tahun.</li>
                    <li>Pastikan data lahir balita sudah tercatat di menu <strong>Data Warga</strong>.</li>
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-between mb-4">
                    <div className="relative w-full sm:w-80">
                        ...
                        <input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                            className="block w-full p-2 pl-9 text-sm border border-gray-300 rounded-lg bg-gray-50 focus:ring-orange-500 focus:border-orange-500"
                            placeholder="Cari nama, NIK, nama ayah/ibu..." />
                    </div>
                    ...
                    <table className="w-full text-xs text-left text-gray-500">
                        <thead className="text-[10px] text-gray-700 uppercase bg-gray-50/80 border-y border-gray-200">
                            <tr>
                                ...
                                <th className="px-3 py-3 text-center">Nama Balita</th>
                                <th className="px-3 py-3 text-center">No KK</th>
                                <th className="px-3 py-3 text-center">Jenis Kelamin</th>
                                <th className="px-3 py-3 text-center">Usia</th>
                                <th className="px-3 py-3 text-center">Nama Ayah</th>
                                <th className="px-3 py-3 text-center">Nama Ibu</th>
                                <th className="px-3 py-3 text-center">Gol. Darah</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filtered.length === 0 ? (
                                <tr><td colSpan={9} className="px-3 py-8 text-center text-gray-500">Tidak ada data balita ditemukan.</td></tr>
                            ) : (
                                currentData.map((w, i) => (
                                    <tr key={w.id} className="bg-white border-b hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-3 py-2 text-center">{w.kartu_keluarga?.nomor_kk || '-'}</td>
                                        <td className="px-3 py-2 text-center">{w.jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan'}</td>
                                        <td className="px-3 py-2 text-center">{formatUsia(w.usia_tahun, w.usia_bulan)}</td>
                                        <td className="px-3 py-2 text-center capitalize">{w.nama_ayah || '-'}</td>
                                        <td className="px-3 py-2 text-center capitalize">{w.nama_ibu || '-'}</td>
                                        <td className="px-3 py-2 text-center">{w.golongan_darah || '-'}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
