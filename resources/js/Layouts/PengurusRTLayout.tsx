...
export default function PengurusRTLayout({ children, title }: { children: React.ReactNode; title?: string }) {
    ...
    const sidebarLinks: SidebarLink[] = [
        {
            name: 'Dashboard',
            href: route('pengurus-rt.dashboard'),
            icon: (
    ...
    const dataWargaLinks = [
        { name: 'Rumah', href: route('pengurus-rt.rumah.index') },
        { name: 'Keluarga', href: route('pengurus-rt.keluarga.index') },
        { name: 'Warga', href: route('pengurus-rt.warga.semua') },
    ];

    const mutasiWargaLinks = [
        { name: 'Kelahiran', href: route('pengurus-rt.mutasi-warga.kelahiran') },
        { name: 'Kematian', href: route('pengurus-rt.mutasi-warga.kematian') },
        { name: 'Pindah Datang', href: route('pengurus-rt.mutasi-warga.pindah-datang') },
        { name: 'Pindah Pergi', href: route('pengurus-rt.mutasi-warga.pindah-pergi') },
    ];

    const kelompokSasaranLinks = [
        { name: 'Pemilih', href: route('pengurus-rt.kelompok-sasaran.pemilih') },
        { name: 'Balita', href: route('pengurus-rt.kelompok-sasaran.balita') },
        { name: 'Lansia', href: route('pengurus-rt.kelompok-sasaran.lansia') },
        { name: 'Ibu Menyusui', href: route('pengurus-rt.kelompok-sasaran.ibu-menyusui') },
        { name: 'Ibu Hamil', href: route('pengurus-rt.kelompok-sasaran.ibu-hamil') },
    ];

    // Keuangan RT: TANPA Arus Kas RW
    const keuanganLinks = [
        { name: 'Arus Kas RT', href: route('pengurus-rt.arus-kas-rt.index') },
        { name: 'Metode Transaksi', href: route('pengurus-rt.metode-transaksi.index') },
    ];
            ...
            <aside className={`fixed inset-y-0 left-0 z-40 bg-white border-r border-gray-200 flex flex-col transform transition-all duration-300 ease-in-out lg:relative lg:translate-x-0 w-64 ${sidebarWidth} ${mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <nav className="px-3 py-4 space-y-1 overflow-y-auto flex-1 scrollbar-hide">
                    ...
                    <div>
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ${keuanganOpen && desktopSidebarOpen ? 'max-h-32' : 'max-h-0'}`}>
                            <div className="ml-9 mt-1 space-y-1 border-l-2 border-gray-100 pl-3">
                    ...
                    {/* ❌ Papan Informasi — dihapus untuk RT */}
                    {/* ❌ Kelola Pengurus — dihapus untuk RT */}
                </nav>
            </aside>
                        ...
                        <Menu as="div" className="relative inline-block text-left z-50 flex-shrink-0">
                            <Menu.Button className="flex items-center gap-2 sm:gap-3 w-full cursor-pointer hover:bg-gray-50 p-1.5 sm:pr-2 rounded-xl transition-colors border border-transparent focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500">
                                ...
                                <div className="flex flex-col items-start leading-tight max-w-[100px] sm:max-w-[200px]">
                                    ...
                                    <span className="text-[10px] sm:text-sm text-gray-600 truncate w-full text-left">Pengurus RT</span>
                                </div>
