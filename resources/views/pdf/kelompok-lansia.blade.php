...
<head>
    <meta charset="UTF-8">
    <title>Data Lansia RW</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'DejaVu Sans', sans-serif; font-size: 9px; color: #1f2937; background: #fff; }
        .header { text-align: center; padding: 14px 0 10px; border-bottom: 2px solid #d97706; margin-bottom: 12px; }
        .header h1 { font-size: 14px; font-weight: bold; letter-spacing: 0.5px; color: #1f2937; }
        .header h2 { font-size: 10px; color: #d97706; margin-top: 2px; }
        ...
        thead tr { background-color: #d97706; color: #ffffff; }
        thead th { padding: 6px 4px; text-align: center; font-size: 8px; font-weight: bold; border: 1px solid #b45309; }
...
@endphp
    ...
    <div class="header">
        <h1>DATA KELOMPOK LANSIA</h1>
        <h2>Kelompok Sasaran â€” Warga Usia â‰¥ 60 Tahun</h2>
    ...
    <div class="meta">
        ...
        <span>Total Data: {{ $wargas->count() }} Lansia</span>
    </div>
        ...
        <tbody>
            @forelse($wargas as $i => $w)
                <tr>
                    ...
                    <td class="center">{{ $w->agama ?: '-' }}</td>
                    <td class="center">{{ $w->golongan_darah ?: '-' }}</td>
                    <td class="center">{{ $w->no_whatsapp ?: '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="9" class="center" style="padding: 20px; color: #9ca3af;">Belum ada data Lansia.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
