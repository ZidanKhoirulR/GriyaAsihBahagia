...
<head>
    <meta charset="UTF-8">
    <title>Daftar Pindah Pergi</title>
    <style>
...
@endphp
    <div class="header">
        <h1>DAFTAR MUTASI PINDAH PERGI WARGA</h1>
    </div>

    <table class="meta-table">
        <tr>
            <td>Tanggal cetak: {{ \Carbon\Carbon::now('Asia/Jakarta')->isoFormat('D MMMM YYYY, HH:mm') }} WIB</td>
            <td class="meta-right">Total data: {{ $warga->count() }} Data Pindah Pergi</td>
        </tr>
    </table>
    ...
    <table class="data-table">
        <thead>
            <tr>
                ...
                <th style="width:10%">Tgl Pindah Pergi</th>
                <th style="width:13%">Pindah Pergi Ke</th>
        ...
        <tbody>
            @forelse($warga as $i => $w)
                ...
                <tr>
                    ...
                    <td class="center">{{ $w->tanggal_pindah_pergi ? \Carbon\Carbon::parse($w->tanggal_pindah_pergi)->isoFormat('D MMMM YYYY') : '-' }}</td>
                    <td>{{ $w->pindah_pergi ?: '-' }}</td>
                    <td>{{ $w->catatan ?: '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="11" class="center" style="padding: 20px; color: #555;">Belum ada data pindah pergi.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
