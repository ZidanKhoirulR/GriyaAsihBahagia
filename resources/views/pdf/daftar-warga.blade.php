...
<head>
    <meta charset="UTF-8">
    <title>Daftar Semua Warga RW</title>
    <style>
        ...
        <tbody>
            @forelse($wargas as $i => $w)
                <tr>
                    ...
                    <td class="center">
                        {{ $w->tempat_lahir }},<br>
                        {{ $w->tanggal_lahir ? \Carbon\Carbon::parse($w->tanggal_lahir)->isoFormat('D MMMM YYYY') : '-' }}
                    </td>
                    <td class="center">{{ $w->jenis_kelamin === 'L' ? 'Laki-laki' : 'Perempuan' }}</td>
                    <td class="center">{{ $w->agama }}</td>
                    <td class="center">{{ $w->jenis_pekerjaan }}</td>
                    <td class="center">{{ $w->shdk }}</td>
                    <td>{{ $w->catatan ?: '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="11" class="center" style="padding: 20px; color: #9ca3af;">Belum ada data Warga.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
