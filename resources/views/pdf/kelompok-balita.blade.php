...
<head>
    <meta charset="UTF-8">
    <title>Data Balita RW</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'DejaVu Sans', sans-serif; font-size: 9px; color: #1f2937; background: #fff; }
        .header { text-align: center; padding: 14px 0 10px; border-bottom: 2px solid #f97316; margin-bottom: 12px; }
        .header h1 { font-size: 14px; font-weight: bold; letter-spacing: 0.5px; color: #1f2937; }
        .header h2 { font-size: 10px; color: #f97316; margin-top: 2px; }
        ...
        thead tr { background-color: #f97316; color: #ffffff; }
        thead th { padding: 6px 4px; text-align: center; font-size: 8px; font-weight: bold; border: 1px solid #ea580c; }
...
@endphp
    ...
    <div class="header">
        <h1>DATA KELOMPOK BALITA</h1>
        <h2>Kelompok Sasaran â€” Warga Usia 0 â€“ 5 Tahun</h2>
    ...
    <div class="meta">
        ...
        <span>Total Data: {{ $wargas->count() }} Balita</span>
    </div>
                ...
                <tr>
                    ...
                    <td class="center">
                        @if($w->usia_tahun == 0)
                            {{ $w->usia_bulan }} bln
                        @else
                            {{ $w->usia_tahun }} thn {{ $w->usia_bulan % 12 }} bln
                        @endif
                    </td>
                    <td style="text-transform: capitalize;">{{ $w->nama_ayah ?: '-' }}</td>
                    <td style="text-transform: capitalize;">{{ $w->nama_ibu ?: '-' }}</td>
                    <td class="center">{{ $w->golongan_darah ?: '-' }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="9" class="center" style="padding: 20px; color: #9ca3af;">Belum ada data Balita.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
